...
local function OpenEntranceMenu()
    ...
    headerMenu[#headerMenu + 1] = {
        ...
        txt = '',
        params = {
            event = 'rx-menu:client:closeMenu'
        }
    }

    exports['rx-menu']:openMenu(headerMenu)
end
...
local function RegisterApartmentEntranceZone(apartmentID, apartmentData)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and not InApartment then
            exports['rx-core']:DrawText(Lang:t('text.options'), 'left')
        else
            exports['rx-core']:HideText()
        end
...
local function RegisterApartmentEntranceTarget(apartmentID, apartmentData)
    ...
    exports['rx-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
...
local function RegisterInApartmentZone(targetKey, coords, heading, text)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside and text then
            exports['rx-core']:DrawText(text, 'left')
        else
            exports['rx-core']:HideText()
        end
...
local function RegisterInApartmentTarget(targetKey, coords, heading, options)
    ...
    local boxName = 'inApartmentTarget_' .. targetKey
    exports['rx-target']:AddBoxZone(boxName, coords, 1.5, 1.5, {
        name = boxName,
        heading = heading,
        minZ = coords.z - 1.0,
        maxZ = coords.z + 5.0,
...
local function DeleteApartmentsEntranceTargets()
    if Apartments.Locations and next(Apartments.Locations) then
        for id, apartment in pairs(Apartments.Locations) do
            if UseTarget then
                exports['rx-target']:RemoveZone('apartmentEntrance_' .. id)
            else
...
local function DeleteInApartmentTargets()
    ...
    if InApartmentTargets and next(InApartmentTargets) then
        for id, apartmentTarget in pairs(InApartmentTargets) do
            if UseTarget then
                exports['rx-target']:RemoveZone('inApartmentTarget_' .. id)
            else
...
local function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Wait(250)
    RXCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            RXCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['rx-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                RangDoorbell = nil
                Wait(500)
                TriggerEvent('rx-weathersync:client:DisableSync')
                Wait(100)
                TriggerServerEvent('rx-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('rx-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('rx-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_close', 0.1)
            TriggerServerEvent('apartments:server:setCurrentApartment', CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('rx-interior:client:SetNewState', true)
            else
                TriggerEvent('rx-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('rx-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_open', 0.1)
    openHouseAnim()
    TriggerServerEvent('rx-apartments:returnBucket')
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do Wait(10) end
    exports['rx-interior']:DespawnInterior(HouseObj, function()
        TriggerEvent('rx-weathersync:client:EnableSync')
        ...
        TriggerServerEvent('rx-apartments:server:SetInsideMeta', CurrentApartment, false)
...
local function SetClosestApartment()
    ...
    if current ~= ClosestHouse and LocalPlayer.state.isLoggedIn and not InApartment then
        ...
        RXCore.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
            DeleteApartmentsEntranceTargets()
            DeleteInApartmentTargets()
        end, ClosestHouse)
...
function MenuOwners()
    RXCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            RXCore.Functions.Notify(Lang:t('error.nobody_home'), 'error', 3500)
            CloseMenuFull()
        else
            local apartmentMenu = {
                {
            ...
            exports['rx-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end

function CloseMenuFull()
    exports['rx-menu']:closeMenu()
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    RXCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('rx-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('rx-spawn:client:openUI', true)
            TriggerEvent('apartments:client:SetHomeBlip', result.type)
        else
            if Apartments.Starting then
                TriggerEvent('rx-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('rx-spawn:client:openUI', true)
            else
                TriggerEvent('rx-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('rx-spawn:client:openUI', true)
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if RangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            RXCore.Functions.Notify(Lang:t('error.to_far_from_door'))
            return
        end
    end
...
RegisterNetEvent('rx-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:SetHomeBlip', function(home)
    CreateThread(function()
        ...
        for name, _ in pairs(Apartments.Locations) do
            ...
            local sprite = (name == home) and 475 or 476
            local coords = Apartments.Locations[name].coords.enter
            ...
            AddTextComponentString(Apartments.Locations[name].label)
            EndTextCommandSetBlipName(blip)
            Apartments.Locations[name].blip = blip
            Wait(0)
        end
    end)
end)
...
RegisterNetEvent('apartments:client:RingDoor', function(player, _)
    ...
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'doorbell', 0.1)
    RXCore.Functions.Notify(Lang:t('info.at_the_door'))
end)
...
RegisterNetEvent('apartments:client:OpenDoor', function()
    if CurrentDoorBell == 0 then
        RXCore.Functions.Notify(Lang:t('error.nobody_at_door'))
        return
    end
...
RegisterNetEvent('apartments:client:ChangeOutfit', function()
    ...
    TriggerEvent('rx-clothing:client:openOutfitMenu')
end)

RegisterNetEvent('apartments:client:Logout', function()
    TriggerServerEvent('rx-houses:server:LogoutLocation')
end)
