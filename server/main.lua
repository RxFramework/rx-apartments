...
RegisterNetEvent('rx-apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
    ...
    if bool then
        ...
        RXCore.Functions.SetPlayerBucket(src, tonumber(routeId))
    else
        ...
        Player.Functions.SetMetaData('inside', insideMeta)
        RXCore.Functions.SetPlayerBucket(src, 0)
    end
end)

RegisterNetEvent('rx-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:openStash', function(CurrentApartment)
    local src = source
    exports['rx-inventory']:OpenInventory(src, CurrentApartment)
end)
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label, firstSpawn)
    ...
    TriggerClientEvent('RXCore:Notify', src, Lang:t('success.receive_apart') .. ' (' .. label .. ')')
    if firstSpawn then
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('RXCore:Notify', src, Lang:t('success.changed_apart'))
    TriggerClientEvent('apartments:client:SetHomeBlip', src, type)
end)
...
RegisterNetEvent('apartments:server:OpenDoor', function(target, apartmentId, apartment)
    local OtherPlayer = RXCore.Functions.GetPlayer(target)
...
RXCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(_, cb, apartment)
    local apartments = {}
...
RXCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(_, cb, apartmentId)
    local retval = 0
...
RXCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(_, cb, apartment)
...
RXCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
RXCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
    local src = source
    local Player = RXCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
RXCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
    local src = source
    local Player = RXCore.Functions.GetPlayer(src)
